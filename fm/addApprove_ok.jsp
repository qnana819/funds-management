<%@page language="java" import="java.util.*" pageEncoding="UTF-8"%>
	...
	if((strproname==null||strproname.length()<=0)||(streconomicSubjects==null||streconomicSubjects.length()<=0)||(strdate==null||strdate.length()<=0)||(strabstract==null||strabstract.length()<=0)||(strexpend==null||strexpend.length()<=0)||(strdepartment==null||strdepartment.length()<=0)||(strapprovalStatus==null||strapprovalStatus.length()<=0)){
		...
		String sql="insert into approve(proname,economicSubjects,date,abstract,expend,department,approvalStatus,time) values('"+strproname+"','"+streconomicSubjects+"','"+strdate+"','"+strabstract+"','"+strexpend+"','"+strdepartment+"','"+strapprovalStatus+"','"+formateOne.format(currentDay)+"')";
		
		stmt.executeUpdate(sql);					
		out.println("添加成功");	
	    out.println("<script>window.alert('添加成功,请跳转到报销列表');location.href='报销列表.jsp'</script>");
		stmt.close();
		conn.close();
